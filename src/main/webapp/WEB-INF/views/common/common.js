$(document).ready(function () {
  ...
  $("#header1").load("/src/main/webapp/WEB-INF/views/common/header.html", function () {
    ...
    if (loginInfo != null) {
      ...
      $(".login-user-info").html(
        '<span class="user-name">이한희</span>님 환영합니다!'
      );
      $(".user-name").css("font-weight", "bold");
      $(".login-user-info").css("margin-right", "50px");
      $(".login-link")
        .parent()
        .attr("href", "/WebContent/login/login.html?type=logout");
...
$(document).on("headerLoaded", function () {
  $("#header1")
    .on("mouseenter", ".menu-link", function () {
      showAllMenus($(this));
      // 지역 선택 modal 열기
...
function showAllMenus(element) {
  ...
  Object.keys(menuData).forEach((key) => {
    ...
    menuData[key].forEach((item) => {
      ...
      let search_add_class =
        item == "역 정보 조회"
          ? "menu-link-sub main-map-info"
          : "menu-link-sub";
      const link = $("<a>")
        .attr("href", search_link)
        .addClass(search_add_class)
        .text(item);
      section.append(link);
    });
